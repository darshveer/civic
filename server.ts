...
app.post("/api/triage", async (req, res) => {
  const { image, mimeType, latitude, longitude, language } = req.body;
  ...
  try {
    ...
    const triageResult = await runTriageAgent(image, mimeType, language || "English");
...
/**
 * API Route: /api/translate — translate cached/stored text (e.g. a stored
 * report description or a previously-generated briefing) into the active UI
 * language. Server-side in-memory cache so repeat views don't re-spend tokens.
 */
...
app.post("/api/translate", async (req, res) => {
  ...
  if (!src.trim() || /^english$/i.test(target)) {
    return res.json({ text: src });
  }
  const key = `${target}::${src}`;
  if (translateCache.has(key)) {
    return res.json({ text: translateCache.get(key), cached: true });
  }
  try {
    const out = await runTranslateAgent(src, target);
    if (translateCache.size > 2000) translateCache.clear(); // simple bound
    translateCache.set(key, out);
    return res.json({ text: out });
  } catch (error: any) {
    console.error("Error in /api/translate:", error);
    return res.json({ text: src }); // fail open
  }
});

/**
 * API Route: /api/intake/extract — staff complaint intake (letters / emails).
 * Accepts either a base64 file (scanned PDF/image) or pasted email text and
 * returns a structured complaint draft for the staff member to review & file.
 */
app.post("/api/intake/extract", async (req, res) => {
  const { fileBase64, mimeType, emailText } = req.body || {};
  if (!fileBase64 && (!emailText || String(emailText).trim().length < 5)) {
    return res
      .status(400)
      .json({ error: "Provide a scanned complaint file or pasted email text." });
  }
  try {
    const draft = await runIntakeExtractionAgent({
    ...
    console.error("Error in /api/intake/extract:", error);
    return res
      .status(500)
      .json({ error: error.message || "Could not extract the complaint." });
  }
});

/**
 * API Route: /api/triage-text — description-only reporting (no image).
 * Runs the Text-Triage Agent (gibberish gate + categorisation) and the Priority
 * engine. Returns 422 with a friendly message when the text is gibberish.
 */
app.post("/api/triage-text", async (req, res) => {
  const { text, language } = req.body;
  if (typeof text !== "string" || text.trim().length < 12) {
    return res.status(400).json({
      error: "Please describe the issue in a full sentence.",
    });
  }
  try {
    const result = await runTextTriageAgent(text, language || "English");
    if (result.isGibberish) {
      return res.status(422).json({
        error:
          result.reason ||
          "That doesn't look like a civic issue — please describe what is wrong and where.",
        isGibberish: true,
      });
    }
    const priority = runPriorityAgent(
    ...
    return res.json({
      ...
      triage: {
        ...
        autoGeneratedDescription: result.autoGeneratedDescription,
        urgencyReasoning: result.urgencyReasoning,
        priorityTier: priority.priorityTier,
        slaTargetHours: priority.slaTargetHours,
      },
    });
  } catch (error: any) {
    console.error("Error in /api/triage-text:", error);
    return res
      .status(500)
      .json({ error: error.message || "Text triage failed." });
  }
});
...
app.post("/api/verify-resolution", async (req, res) => {
  ...
  try {
    console.log("[Resolution Verification] Running analysis + forensics...");
    // Run the before/after fix check AND a Cognitive Forensics authenticity
    // check on the staff "after" photo in parallel — the corporation's proof
    // image is held to the same AI integrity bar as a citizen's report photo.
    const [result, forensics] = await Promise.all([
      runResolutionVerificationAgent(
        beforeImage,
        afterImage,
        mimeType || "image/jpeg",
      ...
      runForensicsAgent(afterImage, mimeType || "image/jpeg").catch(() => null),
    ]);

    const afterFlagged =
      !!forensics &&
      (!forensics.isAuthentic || forensics.fraudConfidenceScore > FRAUD_FLAG_THRESHOLD);

    // A manipulated / inauthentic after-photo can never close an issue, even if
    // it superficially "looks fixed".
    const verification = afterFlagged
      ? {
          isResolved: false,
          confidence: 0,
          notes: `After-photo failed the authenticity check (fraud score ${(forensics!.fraudConfidenceScore * 100).toFixed(0)}%). Upload a genuine, unedited photo of the completed fix.`,
        }
      : result;

    console.log("[Resolution Verification] Done:", { verification, afterFlagged });
    return res.json({
      success: true,
      verification,
      afterForensics: forensics ? { ...forensics, flagged: afterFlagged } : null,
    });
  } catch (error: any) {
...
app.post("/api/predictive-insights", async (req, res) => {
  ...
  try {
    const [insights, hotspots] = [
      await runPredictiveInsightAgent(issues, req.body.language || "English"),
...
app.post("/api/daily-briefing", async (req, res) => {
  ...
  try {
    const briefing = await runDailyBriefingAgent(stats, req.body.language || "English");
...
app.post("/api/draft-petition", async (req, res) => {
  ...
  try {
    const petition = await runPetitionAgent(issue, req.body.language || "English");
...
app.post("/api/missions-coach", async (req, res) => {
  ...
  try {
    const coach = await runMissionsCoachAgent(context || {}, req.body.language || "English");
...
app.post("/api/draft-dispatch", async (req, res) => {
  ...
  if (!issue) return res.status(400).json({ error: "Issue is required." });
  try {
    const dispatch = await runDispatchAgent(issue, req.body.language || "English");
