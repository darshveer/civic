    ...
    const response = await generateWithFallback({
      ...
      contents: [
        ...
        'You are the "Triage Agent" of our C.I.V.I.C. platform, an autonomous civic triage specialist. ' +
          ...
          `Write the autoGeneratedDescription and urgencyReasoning fields in ${language} (keep the category enum and department names in English).`,
      ],
      config: {
...
/**
 * Text-Triage Agent — for description-only reports (no photo). Many citizens
 * can't easily attach a photo, so they describe the issue in words. There is no
 * image to run forensics on, so the ONLY quality gate is a gibberish/quality
 * check; if the text is real, it is categorised/scored like a normal triage.
 * Returns the triage plus an `isGibberish` flag (with a reason) — fails OPEN
 * (treats text as valid) if the model is unavailable.
 */
export async function runTextTriageAgent(
  text: string,
  language: string = "English",
): Promise<TriageResponse & { isGibberish: boolean; reason: string }> {
  ...
  if (clean.length < 12) {
    return {
      ...
      isGibberish: true,
      reason:
        "Please describe the issue in a full sentence (what is wrong and where).",
    };
  }
  try {
    const response = await generateWithFallback({
      ...
      contents: [
        'You are the "Text-Triage Agent" of the C.I.V.I.C. civic platform. A citizen reported an ' +
          "infrastructure issue in words only (no photo). FIRST decide if the text is a genuine, " +
          "intelligible civic complaint (not random characters, spam, or nonsense). If it is gibberish, " +
          "set isGibberish=true and give a short reason. If it is genuine, set isGibberish=false and " +
          "triage it: category, severity 1-10 (SAFETY × PEOPLE × CRITICALITY), a confidence percentage, " +
          "the responsible department, a clean professional description, and a one-line urgency rationale. " +
          `Write the description and rationale in ${language}.\n\nCITIZEN TEXT:\n"""${clean}"""`,
      ],
      config: {
        responseMimeType: "application/json",
        responseSchema: {
          ...
          properties: {
            isGibberish: {
              type: Type.BOOLEAN,
              description:
                "true if the text is nonsense/spam/unintelligible and not a real civic complaint.",
            },
            reason: {
              type: Type.STRING,
              description:
                "If isGibberish, a short friendly reason; otherwise an empty string.",
            },
            category: {
              type: Type.STRING,
              enum: [
            ...
            autoGeneratedDescription: { type: Type.STRING },
    ...
    const parsedText = response.text;
    if (!parsedText) throw new Error("Empty text-triage reply.");
    const raw = JSON.parse(parsedText) as Partial<TriageResponse> & {
      isGibberish?: boolean;
      reason?: string;
    };
    return {
      ...sanitizeTriage(raw),
      isGibberish: Boolean(raw.isGibberish),
      reason: String(raw.reason || ""),
    };
  } catch (err: any) {
    console.error("Text-Triage Agent failed; failing open:", err?.message || err);
    // Fail open: accept the text as a low-confidence "Other" report for humans.
    return {
      category: "Other",
      severityScore: 5,
      confidencePercentage: 30,
      recommendedDepartment: "General Operations",
      autoGeneratedDescription: clean.slice(0, 500),
      urgencyReasoning: "Awaiting manual review (automated triage unavailable).",
      isGibberish: false,
      reason: "",
    };
  }
  ...
  try {
    const response = await generateWithFallback({
      ...
      contents: [
        ...
        Generate 2-3 predictive insights. PREFER predictions grounded in the detected hotspots above (cite the count, location and likely systemic root cause, e.g. a failing road segment, ageing water main, or recurring dumping spot). Each insight needs a concrete recommended action (e.g. "schedule a road-segment inspection"). Write the title, prediction and actionRecommended fields in ${language}.`,
      ],
      config: {
        responseMimeType: "application/json",
        responseSchema: {
  ...
  try {
    const response = await generateWithFallback({
      ...
      contents: [
        ...
        Output a 2-3 sentence engaging briefing. Write entirely in ${language}.`,
      ],
 ...
 */
export async function runPetitionAgent(
  issue: any,
  language: string = "English",
): Promise<{
  title: string;
...
and a brief justification of public impact. Write the title and body in ${language}.`,
      ],
      config: {
        responseMimeType: "application/json",
        responseSchema: {
...
- Write the subject and body in ${language} (keep the reference number and any codes as-is).`,
      ],
      config: {
        responseMimeType: "application/json",
        responseSchema: {
 ...
 */
export async function runMissionsCoachAgent(
  ctx: any,
  language: string = "English",
): Promise<string> {
  try {
...
invent facts beyond the context. Write entirely in ${language}.
...
/**
 * Complaint-Intake Extraction Agent — turns an offline complaint (a scanned
 * letter PDF/image, or a pasted email) into a structured draft a staff member
 * can review and file. Uses Gemini multimodal for PDFs/images (inlineData) and
 * plain text for emails. Fails OPEN with an empty draft so the staff form still
 * opens for manual entry.
 */
export async function runIntakeExtractionAgent(input: {
  ...
  const empty = {
    category: "Other" as CivicCategory,
    description: "",
    locationHint: "",
    complainantName: "",
    complainantPhone: "",
  ...
  try {
    ...
    const source = input.fileBase64
      ? "the attached complaint document (scanned letter)"
      : "the pasted complaint email below";
    parts.push(
      'You are the "Complaint-Intake Agent" for the C.I.V.I.C. municipal platform. ' +
        `Extract a structured civic complaint from ${source}. Identify the civic issue ` +
        "category, a clean professional description of the problem, any location/address mentioned, " +
        "and the complainant's name and phone number if present (else empty strings). Also give a " +
        "severity 1-10. If something is not stated, use an empty string (do not invent)." +
        (input.emailText ? `\n\nEMAIL:\n"""${String(input.emailText).slice(0, 6000)}"""` : ""),
    );
    const response = await generateWithFallback({
      model: "gemini-2.5-flash",
      contents: parts,
      config: {
        ...
        responseSchema: {
          ...
          properties: {
            ...
            complainantPhone: { type: Type.STRING },
            severityScore: { type: Type.INTEGER },
          },
          required: [
            "category",
    ...
    const raw = JSON.parse(response.text || "{}") as Partial<typeof empty>;
    return {
      category: (VALID_CATEGORIES.includes(raw.category as CivicCategory)
        ? raw.category
        : "Other") as CivicCategory,
      description: String(raw.description || ""),
      locationHint: String(raw.locationHint || ""),
      complainantName: String(raw.complainantName || ""),
      complainantPhone: String(raw.complainantPhone || ""),
      severityScore: clampInt(raw.severityScore, 1, 10, 5),
    };
  } catch (err: any) {
    console.error("Intake Extraction Agent failed; returning empty draft:", err?.message || err);
    return empty;
  }
}

// Language NAME → ISO code for the free machine-translation endpoint.
...
/**
 * Translation helper — translates already-generated / cached text (a stored
 * report description, a daily briefing, UI strings, etc.) into the requested
 * language using a FREE, keyless machine-translation endpoint (Google's public
 * translate_a service). This deliberately does NOT use the Gemini LLM, so bulk
 * UI/content translation does not consume AI credits. Returns the input
 * unchanged for English or on any failure (fail-open). Callers should cache.
 */
export async function runTranslateAgent(
  text: string,
  targetLanguage: string,
): Promise<string> {
  const clean = String(text || "").trim();
  if (!clean) return text;
  const code = TRANSLATE_LANG_CODES[String(targetLanguage || "").toLowerCase()];
  if (!code || code === "en") return text;
  try {
    const url =
      "https://translate.googleapis.com/translate_a/single?client=gtx&sl=auto" +
      `&tl=${code}&dt=t&q=${encodeURIComponent(clean.slice(0, 4500))}`;
    const res = await fetch(url);
    if (!res.ok) return text;
    const data: any = await res.json();
    // Response shape: [[[ "translated segment", "source", ... ], ...], ...]
    const out = Array.isArray(data?.[0])
      ? data[0].map((seg: any[]) => (seg && seg[0]) || "").join("")
      : "";
    return out.trim() || text;
  } catch (err: any) {
    console.error("Translate failed; returning original:", err?.message || err);
    return text;
  }
}
